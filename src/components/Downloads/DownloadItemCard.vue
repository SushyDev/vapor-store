<template>
    <v-card class="hover-highlight focus-highlight mt-12 mb-5 mx-6 rounded">
        <v-sheet class="v-sheet--offset mx-auto rounded-lg select-none" color="secondary" elevation="12" max-width="calc(100% - 32px)">
            <v-sparkline fill v-if="$attrs.game.values" :value="$attrs.game.values" smooth="10" color="white" line-width="2" padding="16"></v-sparkline>
            <p v-else>Fetching</p>
        </v-sheet>
        ...
        <v-card-actions>
            ...
            <span class="caption font-weight-light select-none">{{ $attrs.game.progress[0].toFixed(2) }}% - {{ $attrs.game.values[0].toFixed(2) }}MB/s </span>
